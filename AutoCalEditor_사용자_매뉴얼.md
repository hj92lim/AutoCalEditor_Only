...
- **Navigator**: OpCode 기반 구조 탐색
- **C 코드 생성**: 데이터를 기반으로 C 헤더/소스 파일 자동 생성
- **버전 관리**: Git 연동으로 변경 이력 관리
...
2. **이동**: Navigator의 항목을 클릭하면 해당 위치로 이동
3. **검색**: Navigator 상단의 검색창에서 원하는 항목 검색
4. **확장/축소**: `+`, `-` 버튼으로 하위 항목 표시/숨김
